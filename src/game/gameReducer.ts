...
export function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case 'COMPLETE_ROUND_TWO': return state.currentScene === 'CO_LOA_ENTRY' && state.currentChapter === 'chapter-02' && state.completedActivities.includes('chapter-02-memory') && !state.completedChapters.includes('chapter-02') ? { ...state, currentScene: 'ROUND_TWO_COMPLETE', completedChapters: [...state.completedChapters, 'chapter-02'], collectedFragments: state.collectedFragments.includes('fragment-02') ? state.collectedFragments : [...state.collectedFragments, 'fragment-02'] } : state
